...
using Database-backed storage with SQLAlchemy models.

SYLLABUS UNITS COVERED:
- Unit-6: Database Operations (DBMS)
...
class PaymentGateway:
    ...
    DBMS (Unit-6):
    - Stores transactions in database using Transaction model
    ...
    def __init__(self, transactions_file='transactions.txt'):
        ...
        Args:
            transactions_file: Legacy parameter (kept for backward compatibility)
        """
        self.__success_rate = 1.0  # Private attribute (100% success rate - payments always succeed)
        self.transactions_file = transactions_file  # Kept for backward compatibility
    ...
    def process_payment(self, amount, payment_method, user_id, description=""):
        """
        Process a payment with success rate simulation.
        ...
        # Simulate success rate using random (Unit-7)
    ...
    def save_transaction(self, txn_data):
        ...
        Raises:
            CustomException: If database write fails
        """
        try:
            from models import db, Transaction as TransactionModel
            ...
            txn = TransactionModel(
                transaction_id=txn_data.get('id', self.generate_transaction_id()),
                user_id=int(txn_data.get('user_id', 0)),
                username=txn_data.get('username'),
                amount=float(txn_data.get('amount', 0)),
                method=txn_data.get('method', 'wallet'),
                status=txn_data.get('status', 'success'),
                txn_type=txn_data.get('type'),
                description=txn_data.get('description', ''),
                new_balance=txn_data.get('new_balance'),
                txn_date=txn_data.get('date', datetime.now().strftime('%Y-%m-%d')),
                txn_time=txn_data.get('time', datetime.now().strftime('%H:%M:%S')),
                timestamp=timestamp
            )
            db.session.add(txn)
            db.session.commit()
    ...
    def get_transaction(self, txn_id, user_id=None):
        """
        Retrieve a specific transaction by ID from database.
        
        DBMS (Unit-6):
        - Queries Transaction table by transaction_id
        ...
        try:
            ...
            query = TransactionModel.query.filter_by(transaction_id=txn_id)
    ...
    def get_user_transactions(self, user_id):
        """
        Get all transactions for a specific user from database.
        
        DBMS (Unit-6):
        - Queries Transaction table filtered by user_id
    ...
    def get_all_transactions(self):
        ...
        try:
            ...
            transactions = TransactionModel.query.order_by(
...
class WalletManager:
    ...
    DBMS (Unit-6):
    - Stores wallet data in database using Wallet model
    - Uses SQLAlchemy for data persistence
    """
    ...
    def __init__(self, wallet_file='wallets.txt', payment_gateway=None):
        ...
        Args:
            wallet_file: Legacy parameter (kept for backward compatibility)
            payment_gateway: PaymentGateway instance for transactions
        """
        self.wallet_file = wallet_file  # Kept for backward compatibility
    ...
    def get_balance(self, user_id):
        ...
        try:
            ...
            wallet = WalletModel.query.filter_by(user_id=int(user_id)).first()
            if wallet:
    ...
    def create_wallet(self, user_id, initial_balance=0):
        ...
        """
        try:
            from models import db, Wallet as WalletModel
            ...
            from models import db
            db.session.rollback()
            raise CustomException(f"Error creating wallet: {e}")
    ...
    def add_money(self, user_id, amount, payment_method='card', description='Wallet Recharge'):
        ...
        if txn_result['status'] == 'success':
            try:
                ...
                db.session.commit()
                txn_result['new_balance'] = float(wallet.balance)
            except Exception as e:
                from models import db
                db.session.rollback()
                raise CustomException(f"Error updating wallet: {e}")
